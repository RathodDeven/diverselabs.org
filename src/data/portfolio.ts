...
export const portfolioCaseStudies: Doc[] = [
  ...
  /* ───────────────────────── 5 · CDMO VOC + AI roadmap ───────────────────────── */
  {
    ...
    dek: 'A global API contract manufacturer engaged a senior team for a business-optimization program, an active, confidential Voice-of-Customer effort and a quality-system review.',
  ...
  /* ───────────────────────── 13 · Pricing & trade analytics (BI) ───────────────────────── */
  {
    ...
    dek: 'A CPG pricing & trade analytics client needed a multi-project BI solution, and the ability to maintain and extend it independently.',
        ...
        "items": [
          {
            ...
            "body": "For single-source reagents, the client could secure direct pricing and tiered agreements by aggregating demand across CDMOs and process types, and recommended aggregating demand to pursue tiered or fixed pricing on its costliest single-source reagents."
          },
          {
...
const CONTRIBUTIONS: Record<string, string> = {
  'gene-therapy-launch-readiness':
    'I did the hands-on supply-chain work behind this readiness assessment: the activity maps and functional timelines, the demand and capacity forecast, the clinical supply plan, and the QA-release model the team stood up. I also produced the launch playbook through its iterations and the gap analysis. The framework was mine to build and write, not just review.',
  'new-product-introduction-playbook':
    'I authored the NPI playbook itself, building it from the company\'s own launch history into the chapter structure, the backward-counting stage-gate timeline, the activity and RACI checklists, and the launch scenarios. I also built the supporting analysis it rests on: the functional timelines, the demand-and-capacity scenarios, the clinical supply plan timeline, and the release-throughput model. It was mine to write, structure, and model end to end.',
  'materials-internalization-playbook':
    'I ran the make-versus-buy assessment hands-on: baselining the materials and demand picture, building the QA release model, modelling the scenarios from fully outsourced to a client-owned warehouse, and mapping the capabilities each would need. I produced the gap analysis and the material-management slides that fed it, and the go / no-go recommendation was mine to build.',
  ...
  'commercial-packaging-labeling-ndc-strategy':
    'I built the packaging and NDC materials at the centre of this work: the pack-and-label slides, the NDC and serialization-readiness slides, and the bottle, blister and distribution analysis that fed the network design. I also reviewed the RFP and produced the packaging renders and guidance deck, so the vendor screen and the US-versus-EU model had concrete artefacts to decide against.',
  'oral-oncology-blister-commercialization':
    'I worked the packaging-format question hands-on: the configuration and materials analysis, the comparator benchmark, the packaging and stability slides, and the validation-and-stability plan that mapped the blister presentation to a commercial timeline. I mapped the end-to-end sequence on a collaborative whiteboard so the format decision, materials lock and validation steps lined up against the manufacturing window.',
  'cdmo-voice-of-customer-ai-roadmap':
    'I ran the Voice-of-Customer interviews myself and built the quality material behind the diagnosis. I co-authored the structured interview questionnaire, scored each engagement across the nine dimensions, captured the why behind every score, and turned the won-versus-lost contrast and the quality findings into the operating-model recommendations.',
  'rnd-quality-operating-model':
    'I built the maturity model matrix the leadership team scored the current operating model against, and I wrote the functional-expert interview summaries (embedded and non-embedded functional areas) plus the quotes and count slides that calibrated each score, so the baseline reflected what people actually said. I also produced the connect-workshop deck, the guiding principles, scoring criteria, tree map and activity heatmap, that turned the assessment into something leadership could react to live.',
  'external-manufacturing-network-resilience':
    'I built the risk heat map that scored the network, iterating the impact-versus-vulnerability scoring template through several versions and weighting schemes, including a geographic-concentration dimension, and I turned each pass into the board decks (static and dynamic versions) the technical-operations team carried into the readout.',
  'qms-governance-dashboards':
    'I built the QMS governance dashboard solution on an enterprise low-code application platform, designing the screens and logic, wiring a low-code data tool into the backend, and instantiating one repeatable board layout per module plus the cross-module roll-up. I wrote the multi-version training manual and ran the platform enablement so the quality team could run and extend it without us.',
  'cmc-portfolio-prioritization':
    'I produced the capacity-and-loading view at the heart of this work, the per-person FTE and budget graphs, plus the workshop and post-workshop decks the leadership team aligned around: timelines, executive summary, stakeholder and vendor mapping, risk and gaps, and the serialization and PPQ-scenario prep that fed the launch workshop.',
  'pricing-trade-analytics-bi':
    'I built the multi-project pricing and sales-analytics BI from the data model up: the retention-rate, single-time-buyer and repeat-versus-new-buyer views, the sales-performance and price-volume-mix (PVM) waterfall dashboards, and the reconciliation checks that validated every calculation against the source data, plus the project-by-project training manual so the team could extend it without me.',
};
for (const d of portfolioCaseStudies) {
